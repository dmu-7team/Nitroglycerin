            ...
            public void CmdFiredWeapon(Kit_PlayerBehaviour target, int damageIndex, Vector3 originPoint, Vector3 hitPoint)
            {
                ...
                if (target)
                {
                    ...
                    if (compensator)
                    {
                        ...
                        if (compensator.RaycastCheck(this.connectionToClient, originPoint, hitPoint, pb.gameInformation.lagCompensationTolerancePercent, target.weaponHitLayers, out hit))
                        {
                            ...
                            //if (hit.collider.GetComponent<Kit_PlayerDamageMultiplier>() && Kit_IngameMain.instance.currentGameModeType == 2)
                            {
                                ...
                                //if (hit.collider.transform.root.GetComponent<Kit_PlayerBehaviour>())
                                {
                                    ...
                                    if (Kit_IngameMain.instance.currentPvPGameModeBehaviour.ArePlayersEnemies(pb, target))
                                    {
                                        ...
                                        if (!target.spawnProtection || target.spawnProtection.CanTakeDamage(target))
                                        {
                                            ...
                                            if (pb.isServer)
                                            {
                                                target.ServerDamage((behaviour as Kit_ModernWeaponScript).GetDamage(this, Vector3.Distance(pb.playerCameraTransform.position, hit.point)) * pdm.damageMultiplier, id, pb.transform.position, dir, ragdollForce, hit.point, /*pdm.ragdollId*/ 0, pb.isBot, pb.id);
