            ...
            private void Start()
            {
                for (int i = 0; i < menuManager.game.allSingleplayerGameModes.Length; i++)
                {
                    ...
                    if (gameMode.menuPrefab)
                    {
                        ...
                        {
                            Debug.Log("[Singleplayer] Game Mode " + gameMode.name + " has no menu script on its prefab", menu);
                        }
                    }
                    else
                    {
                        Debug.Log("[Singleplayer] Game Mode " + gameMode.name + " has no menu", gameMode);
                    }
                }
            }
        }
