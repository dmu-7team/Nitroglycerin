    ...
    public class Kit_VotingUIDefault : Kit_VotingUIBase
    {
        ...
        public override void OpenVotingMenu()
        {
            if (Kit_IngameMain.instance.currentPvPGameModeBehaviour.CanStartVote())
            {
                ...
                {
                    Kit_IngameMain.instance.DisplayMessage(string.Format(wait.GetLocalizedString(), (lastVote - Time.time).ToString("F0")));
                    BackToPauseMenu();
                }
            }
            else
            {
                Kit_IngameMain.instance.DisplayMessage(impossible.GetLocalizedString());
                BackToPauseMenu();
            }
        }
        ...
        public void KickPlayer()
        {
            ...
            {
                BackToCategory();
                Kit_IngameMain.instance.DisplayMessage(alone.GetLocalizedString());
            }
        }
        ...
        public void ChangeMap()
        {
            if (Kit_GameSettings.currentNetworkingMode == KitNetworkingMode.Traditional)
            {
                ...
                {
                    Kit_IngameMain.instance.DisplayMessage(oneMap.GetLocalizedString());
                    BackToCategory();
                }
            }
        ...
        public void ChangeGameMode()
        {
            if (Kit_GameSettings.currentNetworkingMode == KitNetworkingMode.Traditional)
            {
                if (Kit_IngameMain.instance.gameInformation.allPvpGameModes.Length > 1)
                {
                    ...
                    for (uint i = 0; i < Kit_IngameMain.instance.gameInformation.allPvpGameModes.Length; i++)
                    {
                        ...
                        if (i != Kit_IngameMain.instance.currentGameMode)
                        {
                            ...
                            uint current = i; //This is necessary, otherwise 'i' would change.
                                             //Set name
                            entry.text.text = Kit_IngameMain.instance.gameInformation.allPvpGameModes[i].gameModeName.GetLocalizedString();
                            //Add delegate
                ...
                {
                    Kit_IngameMain.instance.DisplayMessage(oneGamemode.GetLocalizedString());
                }
            }
        }
            ...
            if (voting)
            {
                ...
                if (voteDescription)
                {
                    ...
                    else if (voting.votingOn == Kit_VotingBase.VotingOn.Map)
                    {
                        if (Kit_GameSettings.currentNetworkingMode == KitNetworkingMode.Traditional)
                        {
                            voteDescription.text = string.Format(switchMap.GetLocalizedString(), Kit_IngameMain.instance.currentPvPGameModeBehaviour.traditionalMaps[voting.argument].mapName);
                        }
                        else if (Kit_GameSettings.currentNetworkingMode == KitNetworkingMode.Lobby)
                        {
                            voteDescription.text = string.Format(switchMap.GetLocalizedString(), Kit_IngameMain.instance.currentPvPGameModeBehaviour.lobbyMaps[voting.argument].mapName);
                        }
                    }
                    else if (voting.votingOn == Kit_VotingBase.VotingOn.GameMode)
                    {
                        voteDescription.text = string.Format(switchGamemode.GetLocalizedString(), Kit_IngameMain.instance.gameInformation.allPvpGameModes[voting.argument].gameModeName);
                    }
                }
                if (yesVotes)
                {
